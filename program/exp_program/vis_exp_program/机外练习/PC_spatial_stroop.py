...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/dddd1007/Library/CloudStorage/Dropbox/工作/博士工作/ResearchData/project9_fmri_spatial_stroop/program/上右下左/PC_spatial_stroop.py',
...
black_text = visual.TextStim(win=win, name='black_text',
    text=None,
    font='Open Sans',
...
short_rest_image = visual.ImageStim(
    win=win,
    name='short_rest_image', 
    image='material/short_rest.png', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(1.3, 1),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
...
rest_end_image = visual.ImageStim(
    win=win,
    name='rest_end_image', 
    ...
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
rest_end_key_resp = keyboard.Keyboard()
...
for thisPractice_trial in practice_trials:
    ...
    while continueRoutine and routineTimer.getTime() < 1.2:
        ...
        if stim_resp.status == STARTED and not waitOnFlip:
            theseKeys = stim_resp.getKeys(keyList=['q','p'], waitRelease=False)
    ...
    while continueRoutine:
        ...
        # *black_text* updates
        if black_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            black_text.frameNStart = frameN  # exact frame index
            black_text.tStart = t  # local t and not account for scr refresh
            black_text.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(black_text, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'black_text.started')
            black_text.setAutoDraw(True)
        if black_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > black_text.tStartRefresh + iti_duration-frameTolerance:
                # keep track of stop time/frame for later
                black_text.tStop = t  # not accounting for scr refresh
                black_text.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'black_text.stopped')
...
for thisRun in runs:
    ...
    for thisBlock in blocks:
        ...
        # update component parameters for each repeat
        # keep track of which components have finished
        short_restComponents = [short_rest_image]
        for thisComponent in short_restComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine:
            ...
            if short_rest_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                short_rest_image.frameNStart = frameN  # exact frame index
                short_rest_image.tStart = t  # local t and not account for scr refresh
                short_rest_image.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(short_rest_image, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'short_rest_image.started')
                short_rest_image.setAutoDraw(True)
            if short_rest_image.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > short_rest_image.tStartRefresh + short_rest_duration-frameTolerance:
                    # keep track of stop time/frame for later
                    short_rest_image.tStop = t  # not accounting for scr refresh
                    short_rest_image.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'short_rest_image.stopped')
        ...
        routineForceEnded = False
        # update component parameters for each repeat
        rest_end_key_resp.keys = []
        rest_end_key_resp.rt = []
        _rest_end_key_resp_allKeys = []
        # Run 'Begin Routine' code from reset_jitter_code
        shuffle(jitter_list)
        count_num = 0
        # keep track of which components have finished
        rest_endComponents = [rest_end_image, rest_end_key_resp]
        ...
        while continueRoutine:
            ...
            if rest_end_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                rest_end_image.frameNStart = frameN  # exact frame index
                rest_end_image.tStart = t  # local t and not account for scr refresh
                rest_end_image.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(rest_end_image, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'rest_end_image.started')
                rest_end_image.setAutoDraw(True)
            if rest_end_image.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rest_end_image.tStartRefresh + stop_rest_duration-frameTolerance:
                    # keep track of stop time/frame for later
                    rest_end_image.tStop = t  # not accounting for scr refresh
                    rest_end_image.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'rest_end_image.stopped')
                    rest_end_image.setAutoDraw(False)
            
            # *rest_end_key_resp* updates
            waitOnFlip = False
            if rest_end_key_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                rest_end_key_resp.frameNStart = frameN  # exact frame index
                rest_end_key_resp.tStart = t  # local t and not account for scr refresh
                rest_end_key_resp.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(rest_end_key_resp, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'rest_end_key_resp.started')
                rest_end_key_resp.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(rest_end_key_resp.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(rest_end_key_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if rest_end_key_resp.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rest_end_key_resp.tStartRefresh + stop_rest_duration-frameTolerance:
                    # keep track of stop time/frame for later
                    rest_end_key_resp.tStop = t  # not accounting for scr refresh
                    rest_end_key_resp.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'rest_end_key_resp.stopped')
                    rest_end_key_resp.status = FINISHED
            if rest_end_key_resp.status == STARTED and not waitOnFlip:
                theseKeys = rest_end_key_resp.getKeys(keyList=['space'], waitRelease=False)
                _rest_end_key_resp_allKeys.extend(theseKeys)
                if len(_rest_end_key_resp_allKeys):
                    rest_end_key_resp.keys = _rest_end_key_resp_allKeys[-1].name  # just the last key pressed
                    rest_end_key_resp.rt = _rest_end_key_resp_allKeys[-1].rt
                    # a response ends the routine
                    continueRoutine = False
        ...
        for thisComponent in rest_endComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # check responses
        if rest_end_key_resp.keys in ['', [], None]:  # No response was made
            rest_end_key_resp.keys = None
        blocks.addData('rest_end_key_resp.keys',rest_end_key_resp.keys)
        if rest_end_key_resp.keys != None:  # we had a response
            blocks.addData('rest_end_key_resp.rt', rest_end_key_resp.rt)
    ...
    # update component parameters for each repeat
    # keep track of which components have finished
    long_restComponents = [long_rest_image]
    ...
    while continueRoutine:
        ...
        if long_rest_end_image.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > long_rest_end_image.tStartRefresh + stop_long_rest_duration-frameTolerance:
                # keep track of stop time/frame for later
                long_rest_end_image.tStop = t  # not accounting for scr refresh
                long_rest_end_image.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'long_rest_end_image.stopped')
        ...
        if long_rest_end_resp.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > long_rest_end_resp.tStartRefresh + stop_long_rest_duration-frameTolerance:
                # keep track of stop time/frame for later
                long_rest_end_resp.tStop = t  # not accounting for scr refresh
                long_rest_end_resp.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'long_rest_end_resp.stopped')
                long_rest_end_resp.status = FINISHED
